...
if ($hyperv -and -not $hyperv.HypervisorPresent) {
    ...
    Write-Host "`n  To enable virtualization:" -ForegroundColor Yellow
    Write-Host "  1. Restart your PC and enter BIOS/UEFI (usually F2, Del, or F10 during boot)" -ForegroundColor Yellow
    Write-Host "  2. Look for: Intel VT-x / AMD-V / SVM / Virtualization Technology" -ForegroundColor Yellow
    Write-Host "  3. Enable it" -ForegroundColor Yellow
    Write-Host "  4. Save and boot back into Windows" -ForegroundColor Yellow
    Write-Host "  5. Run this script again`n" -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "`n  A new Ubuntu window will open. Wait for it to finish, create your Linux" -ForegroundColor Yellow
    Write-Host "  username/password, then close it and press Enter here.`n" -ForegroundColor Yellow
