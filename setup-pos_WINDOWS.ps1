...
.SYNOPSIS
  ...
  Run this AFTER setup-wsl_WINDOWS.ps1 + restart.
...
if ($distros -notcontains $wslDistro) {
    Fail "WSL distro '$wslDistro' not found. Run setup-wsl_WINDOWS.ps1 first."
}
...
if ($LASTEXITCODE -ne 0) {
    Fail "Docker not found inside WSL. Run setup-wsl_WINDOWS.ps1 first."
}
